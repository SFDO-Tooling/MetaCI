...
def scratch_org(request):
    planrepos = PlanRepository.objects.for_user(request.user, 'plan.run_plan')
    planrepos = planrepos.should_run().filter(plan__role='scratch').order_by('repo__name','plan__name')
    if not planrepos.count():
        raise PermissionDenied('You are not authorized to create orgs')
...
def qa_org(request):
    planrepos = PlanRepository.objects.for_user(request.user, 'plan.run_plan')
    planrepos = planrepos.should_run().filter(plan__role='qa').order_by('repo__name','plan__name')
    if not planrepos.count():
        raise PermissionDenied('You are not authorized to create orgs')
