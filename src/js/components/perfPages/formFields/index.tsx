...
const ChoiceField = (filter: FilterDefinition, currentValue: string | null, fetchServerData): Field => {
  ...
  return {
    ...
    currentValue,
    render: () => <FilterPicker key={filter.name} field_name={filter.name} choices={choices_as_objs} currentValue={currentValue} onSelect={fetchServerData} />
  };
};

const CharField = (filter: FilterDefinition, currentValue: string | null, fetchServerData): Field => ({
  name: filter.name,
  currentValue,
  render: () => <TextInput defaultValue={currentValue} label={filter.label} tooltip={filter.description} onValueUpdate={fetchServerData} />
});
...
export const AllFilters = ({
  ...
  return <div key="filterGrid" className="slds-grid slds-wrap slds-gutters">
      {filters.map(filter => <div key={filter.name} className="slds-col slds-size_3-of-12">
          {filter.render()}
        </div>)}
    </div>;
};
